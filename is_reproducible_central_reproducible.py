...
if __name__ == '__main__':
    ...
    for buildspec in mapping:
        ...
        expected_buildcompare_candidate = glob.glob(os.path.join(PATH, groupId, artifactId, version, '*.buildcompare'))
        if len(expected_buildcompare_candidate) == 0:
        ...
        if len(expected_buildcompare_candidate) != 1:
            print(f"Expected one buildcompare file, but found {len(expected_buildcompare_candidate)}, {os.path.join(PATH, groupId, artifactId, version)}")
            sys.exit(1)
        expected_buildcompare = expected_buildcompare_candidate[0]
