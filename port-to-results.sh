...
is_in_csv() {
    ...
    local artifact_id=$2
    local version=$3
    grep -E "^${group_id}:${artifact_id}:${version}$" diffoscope-failures/pseudo-failed.txt
    return $?
}
...
do
    ...
    mkdir -p pseudo-failed/$groupId/$artifactId/$version
    cp $buildspec pseudo-failed/$groupId/$artifactId/$version
